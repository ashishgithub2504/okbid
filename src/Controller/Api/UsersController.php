...
class UsersController extends AppController {
    ...
    public function login() {
        if ($this->request->is('post') && !empty($this->request->data)) {
            ...
            $this->paramsValidation(array('phone' => 'notBlank', 'password' => 'notBlank','type'=>'notBlank','device_token'=>'notBlank','device_type'=>'notBlank'));
...
//                } else 
    ...
    public function register() {
        if ($this->request->is('post')) {
            ...
            if ($this->Users->save($user)) {
                ...
                $this->responseData = $this->_loginResponse($user['id'], $this->request->data, $auth_token);
    ...
    public function resendotp(){
        $this->paramsValidation(array('email' => 'notBlank'));
        $this->request->data['activation_code'] = rand(10000,100000); 
        $result = $this->Users->findByEmail($this->request->data['email'])->select(['prefix','phone'])->first();
        
        $phonenum = $result['prefix'].$result['phone'];
        $this->Users->updateAll(['activation_code'=>$this->request->data['activation_code']], ['email' => $this->request->data['email']]);
        $send = $this->sendmessage($this->request->data['activation_code'],$phonenum);
        if($send == '1'){
            $this->status = true;
            $this->responseData = ['otp'=>$this->request->data['activation_code']];
            $this->message = $this->msgDictonary['signup_success_'.$this->language];
        }
         $this->respond();
    }
...
//            }
    ...
    public function broadcast(){
        $results = $this->Users->find()
                ...
                ->where(['status' => '1','device_token !=' => '','role_id' => '2'])
                ->hydrate(false)
                ->toArray();
